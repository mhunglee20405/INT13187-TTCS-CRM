...
const seed = async () => {
    try {
        ...
        // Seed Tiers
        const tierCount = await Tier_1.default.countDocuments();
        if (tierCount === 0) {
            await Tier_1.default.insertMany([
                {
                    tierId: "TIER_BRONZE",
                    tierName: "Bronze",
                    minExpense: 0,
        ...
        // Seed Memberships
        const membershipCount = await Membership_1.default.countDocuments();
        if (membershipCount === 0) {
            await Membership_1.default.insertMany([
                {
        ...
        // Seed Gifts
        const giftCount = await Gift_1.default.countDocuments();
        if (giftCount === 0) {
            await Gift_1.default.insertMany([
                {
                    giftId: "GIFT001",
        ...
        const receptionistExists = await User_1.default.findOne({ username: "letanvien" });
        if (!receptionistExists) {
            const hashed = await bcryptjs_1.default.hash("letanvien123", 10);
            await User_1.default.create({
                username: "letanvien",
